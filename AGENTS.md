...
11. **Every seeder inserts its own `workspace_membership` rows for every principal it creates.** The `20260617120200` backfill only captures users that existed at MIGRATION time; seed users are inserted *after* migrations run. The US-003 clause is AND-ed with the owner-OR-ACL predicate, so a principal with no membership row sees NOTHING - not chunks it owns, not chunks explicitly granted to it in `chunk_acl`. Seed documents that omit `workspace_id` land in the Default Workspace via the column DEFAULT, so that is the workspace to join. `db_seed/corpus_seed.py` and `db_seed/wikipedia_seed.py` each have an idempotent `_ensure_workspace_membership`; a new seeder without one silently retrieves zero rows.

12. **An eval must refuse to score a run that produced no signal.** "Retrieved nothing" and "retrieved the wrong things" are different outcomes and must never collapse into the same reportable number - a `return 0.0` on an empty gold set publishes a well-formed table indistinguishable from a real regression (that is what hid invariant 11 for two months of nightlies). Guard where the degeneracy is first knowable, not only inside the scoring function, and name the principal / question / knob in the message. `evals/permissions_scale/runner.py::DegenerateRunError` is the pattern; `evals/permissions_scale/test_degenerate_guard.py` pins it.
