...
The PR is opened and set to auto-merge (`gh pr merge --auto --squash`) with the **`EVAL_PUBLISH_TOKEN`** fine-grained PAT (Contents + Pull requests read/write), **not** the default `GITHUB_TOKEN`: GitHub blocks recursive workflow runs, so a PR opened by `GITHUB_TOKEN` would never trigger the required checks and would sit unmergeable forever.
The snapshot commit is pushed to a fresh per-run branch (`eval-publish/<slug>-<date>-<run_id>-<run_attempt>`) so concurrent and re-run jobs never collide, and the "no changes → no PR" guard (`git diff --cached --quiet`) is preserved.
...
Any NEW scheduled workflow that publishes a snapshot must follow this shape, not add a direct push to `main`.
