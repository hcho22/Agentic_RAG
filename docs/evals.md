...
A leg that is **heavily but not entirely** mislabeled still exercises ≥1 row, so it *passes* the positive control, yet it measures the false-resolve ceiling over a shrunken sample that can mask a bad faithfulness gate.
That is a **live** masking path, not a latent one: the P3 gold is now 9 rows, and the 5 that measure below 0.5 (`e7-p3-01/02/03` in the 0.42–0.47 band, `e7-p3-07` at 0.4234, `e7-p3-09` at 0.4570) mislabel together at any swept τ_sim=0.5 point — 56%, over the default ceiling — even though the main leg's default τ_sim of 0.4 currently leaves all 9 above the retrieval gate.
