...
**Scores not yet computed.** `score_with_ragas()` (`evals/retrieval/ragas.py`) is still the US-001 scaffold and returns no scores, so `--include-ragas` and the weekly workflow currently publish an unmeasured RAGAS table - the gate algorithms, `gate.yaml` bindings, and publishing pipeline described below are shipped but stay inert until a later story lands the evaluation pipeline. The weekly page's retrieval metrics and cross-family Claude-judge scores are real measurements.
