...
The PR is opened and set to auto-merge (`gh pr merge --auto --squash --delete-branch`) with the **`EVAL_PUBLISH_TOKEN`** fine-grained PAT (Contents + Pull requests read/write), **not** the default `GITHUB_TOKEN`: GitHub blocks recursive workflow runs, so a PR opened by `GITHUB_TOKEN` would never trigger the required checks and would sit unmergeable forever.
...
If the secret is absent or "Allow auto-merge" is off, the publish step **fails loudly** rather than falling back to a direct push - a silent fallback would defeat the whole ruleset.
When `EVAL_PUBLISH_TOKEN` in particular resolves empty, `gh pr create` would otherwise die with the opaque "gh: ... set the GH_TOKEN environment variable" (exit 4), so each of the five publish steps (the nightly baseline + its `continue-on-error` reranker step, the permissions-scale nightly, and the two weeklies) runs a **named-cause preflight** just before `gh pr create`: an empty token emits an explicit `::error::` annotation naming `EVAL_PUBLISH_TOKEN` and the exact `eval-publish/...` branch the snapshot is now stranded on, then fails the step.
That empty-token lapse is the concrete failure that stranded 17 nights of receipts on push branches unnoticed for a week from 2026-08-12 while `main` froze.
...
It runs under the default `GITHUB_TOKEN`, never the possibly-empty `EVAL_PUBLISH_TOKEN`, so it can still file the alarm even when that token is exactly what is missing, and it never fails the job on its own (a failed publish is already red, and the nightly reranker publish is deliberately `continue-on-error`, so its failure must stay non-blocking); it gates on the publish step's `outcome` so a runner/seed crash is left to the harness-failure step instead.
