...
**An empty gold set scores 0.000 rather than failing loud.** `recall_at_k` returns `0.0` when a question's resolved gold set is empty, so an authoring mistake would be indistinguishable in the table from a real retrieval regression and would drag the gated E4/E6 means down with it. Content anchoring (§2.2) is what keeps this out of reach in practice - `parse_gold_anchors` rejects a question with no anchors, and an anchor that resolves to nothing raises `ZeroResolveError` before any scoring happens - but the guard lives on the authoring path, not in the metric. The sibling permissions-scale runner takes the opposite line and *refuses* the run (`DegenerateRunError`) because its gold is derived from the run itself rather than authored; why the two differ, and why this fixture-backed path was deliberately left as-is, is invariant 12 in `AGENTS.md`.
