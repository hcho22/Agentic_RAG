...
# Eval-harness guards - fully offline (no DB, no secrets, no network), same two
# modules the per-PR `eval-harness-guards` job runs. They pin the rule that a
# harness which measured nothing must refuse to publish a number for it.
